...
const OperationCards = () => {
  return (
    <Container>
      <Row xs={2} md={2} className={styles.body}>
          ...
          <Col key={idx} className="d-flex justify-content-center">
            <Link to={operation.path} style={{ textDecoration: 'none' }}>
              <Card className={`${styles.card} text-center`} style={{ cursor: 'pointer' }}>
                <Card.Img variant="top" className={styles.icon} src={operation.icon} alt={`Icono de ${operation.label}`} style={{ width: '90px', height: '90px' }} />
                <Card.Body>
                  <Card.Title className={styles.cardTitle}>{operation.label}</Card.Title>
                </Card.Body>
              </Card>
            </Link>
          </Col>
