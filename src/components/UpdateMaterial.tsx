...
const UpdateMaterial = () => {
    ...
    const fetchMaterial = async () => {
      if (id) {
        try {
          ...
          setFormData({
            ...material,
            image: null, // Optional: Set default for image if needed
          });
        } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Construir un objeto IMaterial
    const updatedMaterial: IMaterial = {
      id: id || '', // Asegúrate de que id esté definido
      name: formData.name,
      ...
      cost: formData.cost,
      image: formData.image ? URL.createObjectURL(formData.image) : '', // Convierte a string si es necesario
    };

    if (id) {
      try {
        await materialsService.update(id, updatedMaterial); // Pasa el objeto IMaterial
      ...
      setError('ID is undefined. Cannot update product.');
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <input type="number" name="cost" value={formData.cost} onChange={handleChange} placeholder="Cost" required />
